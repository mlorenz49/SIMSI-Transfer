...
class SpectraExtractor:
    ...
    def __init__(self, ms_lvl=2):
        self.scan = None
        self.results = None
        self.ms_level = ms_lvl
    ...
    def extract(self, scan):
        ...
        if self.results["ms_level"] != self.ms_level:
            self.results["precursor_mz"], self.results["precursor_charge"] = (np.NaN, np.NaN)
            self.results["mz_values"], self.results["intensity_values"] = (np.NaN, np.NaN)
        else:
...
class SpectraParser:
    ...
    def __init__(self,
        ...
        if spec_file_format == "mzML":
            ...
            self._extractor = MzMLExtractor(ms_lvl=ms_level)
